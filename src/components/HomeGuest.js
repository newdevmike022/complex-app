...
function HomeGuest() {
    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value.length > 25) {
          ...
          draft.username.message = "Username cannot exceed 25 characters.";
        }
  ...
  }, [state.username.checkCount, dispatch, state.username.value]);
  ...
  }, [state.email.checkCount, dispatch, state.email.value]);
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          appDispatch({ type: "flashMessage", value: "Congratulations! Welcome to your new account." });
        } catch (e) {
          console.log("There was a problem or the request was cancelled.");
        }
      }
